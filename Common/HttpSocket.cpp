...
BOOL CHttpSocket::Socket()
{
	...
	{
		MessageBox(NULL,_T("socket()函数执行失败!"),_T("错误"),MB_OK);
		return FALSE;
	}
...
BOOL CHttpSocket::Connect(char *szHostName,int nPort)
{
	...
	if(m_phostent==NULL)
	{
		MessageBox(NULL,_T("gethostbyname()函数执行失败!"),_T("错误"),MB_OK);
		return FALSE;
	}
	...
	USES_CONVERSION;
	strcpy_s(m_ipaddr,sIP.GetLength()+1,T2A(sIP.GetBuffer()));
	sIP.ReleaseBuffer();
	...
	*/
	...
	if(connect(m_s,(struct sockaddr*)&destaddr,sizeof(destaddr))!=0)
	{
		//CloseSocket();
		//m_s=NULL;
		MessageBox(NULL,_T("connect()函数执行失败!"),_T("错误"),MB_OK);
		return FALSE;
	}
...
BOOL CHttpSocket::SendRequest(const char *pRequestHeader, long Length)
{
	...
	{
		MessageBox(NULL,_T("send()函数执行失败!"),_T("错误"),MB_OK);
		return FALSE;
	}
	int nLength;
...
BOOL CHttpSocket::CloseSocket()
{
	if(m_s != NULL)
	{
		...
		{
			MessageBox(NULL,_T("closesocket()函数执行失败!"),_T("错误"),MB_OK);
			return FALSE;
		}
	}
	m_s = NULL;
...
BOOL CHttpSocket::SetTimeout(int nTime, int nType)
{
	...
	if(dwErr)
	{
		MessageBox(NULL,_T("setsockopt()函数执行失败!"),_T("错误"),MB_OK);
		return FALSE;
	}
	return TRUE;
}
...
int CHttpSocket::GetField(const char *szSession, char *szValue, int nMaxLength)
{
	...
	int nPos = -1;
	nPos = strRespons.Find(CString(szSession,sizeof(szSession)),0);
	if(nPos != -1)
	{
		nPos += strlen(szSession);
		nPos += 2;
		int nCr = strRespons.Find(_T("\r\n"),nPos);
		CString strValue = strRespons.Mid(nPos,nCr - nPos);
		USES_CONVERSION;
		strcpy_s(szValue,strValue.GetLength()+1,T2A(strValue.GetBuffer()));
